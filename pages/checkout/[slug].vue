            ...
            <form>
              <v-row>
                ...
                <v-col cols="12" md="6" class="mt-md-n8">
                  <v-text-field v-model="personal.cpf" hide-spin-buttons class="mt-n8 mt-md-0" label="CPF" required
    ...
    showErrorMessage.text }}</v-alert>
                ...
                <v-card-text>
                  <v-form>
                    <v-row>
                      ...
                      <v-col cols="12" class="mt-md-n8">
                        <v-select :color="color" v-model="selectedInstallments" density="compact"
                          item-value="installment" item-title="itemTitle" variant="solo"
                    ...
                    <v-btn class="me-4 mt-4" :disabled="pending" rounded="xl" @click="getPaymentToken" :color="color"
                      block><span v-if="!pending">PAGAR AGORA</span>

                      <span v-if="pending">PROCESSANDO SEU PAGAMENTO... &nbsp;</span><v-progress-circular size="20"
                        indeterminate :color="color" v-if="pending"></v-progress-circular></v-btn>
                  </v-form>
                </v-card-text>
              </v-card>
      ...
      <v-dialog width="600" persistent v-model="paymentSuccessDialog">
        <v-card rounded="xl" title="Pagamento concluído" subtitle="Em breve você receberá uma mensagem nossa!"
          ...
          :color="color" rounded="xl" block
          @click="paymentSuccessDialog = false"><v-icon>mdi-close</v-icon></v-btn></v-dialog>
