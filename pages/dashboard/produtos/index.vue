<template>
      ...
      <v-row>
        ...
        <v-col cols="12">
          <v-data-table
            ...
            no-data-text="Não existe nenhum produto cadastrado"
            :items="items"
          >
            ...
            <template v-slot:item.checkoutLink="{ item }">
              <v-btn
                rounded="xl"
                :href="'https://socialpro.pro/checkout/' + item.checkoutLink"
                color="primary"
                variant="tonal"
                ...
                <v-col cols="12">
                  ...
                  <v-dialog v-model="dialogEmailContent" width="400">
                    <v-card rounded="xl" color="background">
                      <v-card-title
                        ><v-icon @click="dialogEmailContent = false"
                          >mdi-close</v-icon
                        ></v-card-title
                      >
                      <v-card-text class="text-center">
                        <v-form>
                          <h4 class="mb-n2 text-subtitle-1">
                            Digite o conteúdo do e-mail pós-compra
                          </h4>
                          <h4
                            class="mb-4 text-medium-emphasis text-caption text-justify"
                          >
                            Todos os e-mails serão enviados de forma automático,
                            utilizando este conteúdo como base.
                          </h4>
                          ...
                          <v-file-input
                            ...
                            color="primary"
                            rounded="xl"
                            label="Clique para enviar seu arquivo"
                          ></v-file-input>
                        </v-form>
                      </v-card-text>
                    </v-card>
  ...
  <v-dialog v-model="confirmDeleteProduct" width="400" persistent>
    <v-card
      ...
      subtitle="Quer mesmo deletar o seu produto?"
  ...
  try {
    ...
    if (inline.value === true) {
      ...
      const dataEmail = await $fetch(
        `https://checkout.socialpro.pro/emailContent`,
        {
          method: "post",
          body: emailBody,
        },
...
const deleteProduct = async () => {
  try {
    ...
    const data = await $fetch(
      `https://checkout.socialpro.pro/product/${itemActive.value._id}`,
      {
        method: "delete",
      },
    );
